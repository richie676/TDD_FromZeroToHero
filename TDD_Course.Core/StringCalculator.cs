...
namespace TDD_Course.Core
{
        public class StringCalculator
        {
        public decimal Add(string numbers)
        {
            var parsedInputValues = ParseInput(numbers);
            decimal result = 0m;

            foreach(var parsedInputValue in parsedInputValues)
            {
                result = Decimal.Add(result, parsedInputValue);
            }

            return result;
        }

        private IEnumerable<decimal> ParseInput(string numbers)
        {
            ValidateInput(numbers);

            var inputValues = numbers.Split(',');
            ...
            foreach (var value in inputValues)
            {
                try
                {
                    decimalValues.Add(Decimal.Parse(value.Trim()));
                }
                catch
                {
